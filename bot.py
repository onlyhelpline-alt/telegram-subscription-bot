...
async def start(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🔥 Welcome to VIP Subscription Bot 🚀",reply_markup=InlineKeyboardMarkup(keyboard))
...
async def plans(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text("💎 Choose your subscription plan 👇",reply_markup=InlineKeyboardMarkup(keyboard))
...
async def screenshot_button(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text("📸 Please send payment screenshot here.")
...
📦 Plan: {plan}""",
    ...
    await update.message.reply_text("⏳ Payment received. Please wait for admin approval.")
...
Enjoy premium access 🚀""")
    ...
    await context.bot.send_message(ADMIN_ID,f"✅ User {uid} activated successfully")
...
async def reject(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(uid,"❌ Payment rejected by admin")
...
async def mysub(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text(f"""📦 Your Plan: {r[1]}
📅 Expiry: {r[2]}""")
...
async def setexpiry(update:Update,context:ContextTypes.DEFAULT_TYPE):
    ...
    cur.execute("UPDATE users SET expiry=? WHERE user_id=?",(expiry.strftime("%Y-%m-%d"),uid))
    conn.commit()
