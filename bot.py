...
await context.bot.send_message(ADMIN_ID,f"✅ User {uid} activated")
...
await context.bot.send_message(ADMIN_ID,f"❌ Payment rejected for {uid}")
...
while True:
    ...
    for r in rows:
        ...
        if 23 < hours < 24:
            try:
                await app.bot.send_message(uid,"⚠️ Subscription will expire in 24 hours")
            except:
                pass

        if hours < 0:
            try:
                await app.bot.send_message(uid,"❌ Subscription expired")
            except:
                pass
